 ...
 *
 */
namespace leinad4mind\ppr\controller;
		...
		if ($this->auth->acl_get ( 'u_ppr_view' )) {
			...
			while ( $list = $this->db->sql_fetchrow ( $list_query ) ) {
				...
				if ($this->auth->acl_get ( 'u_ppr_delete' )) {
					$delete = $this->helper->route ( 'leinad4mind_pprdelete_controller', array (
				...
				if ($this->auth->acl_get ( 'u_ppr_restore' )) {
					$restore = $this->helper->route ( 'leinad4mind_pprrestore_controller', array (
							'revision_id' => $list ['revision_id'] 
					) );
				} else {
					$restore = false;
	...
	public function delete($revision_id) {
		if ($this->auth->acl_get ( 'u_ppr_delete' )) {
			if (confirm_box ( true )) {
				...
				$links = "<a href=\"" . $this->helper->route ( 'leinad4mind_ppr_controller', array (
						'post_id' => $list ['post_id'] 
				) ) . "\">" . $this->user->lang ['PPR_RETURN'] . "</a>";
